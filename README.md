...
- `ai-security-platform/` — папка с веб-приложением на Flask:
  - `backend/` — серверная часть (app.py, anomaly_detector.py, database.py, profile_generator.py)
...
## 🚀 Работающее веб-приложение на Render

Ключевым результатом практической части дипломной работы стала разработка и деплой **полнофункционального веб-приложения** для обнаружения аномалий в действиях пользователей.
